...
function extractKeyLevels(points) {
  ...
  for (let i = window; i < points.length - window; i++) {
    ...
    if (isHigh) levels.push({ price: p.high, type: '׳”׳×׳ ׳’׳“׳•׳× ׳§׳¨׳•׳‘׳”' });
    if (isLow) levels.push({ price: p.low, type: '׳×׳׳™׳›׳” ׳§׳¨׳•׳‘׳”' });
  }
  const currentPrice = points[points.length - 1].close;
  let above = levels.filter(l => l.price > currentPrice * 1.01 && l.type.includes('׳”׳×׳ ׳’׳“׳•׳×')).sort((a,b) => a.price - b.price);
  let below = levels.filter(l => l.price < currentPrice * 0.99 && l.type.includes('׳×׳׳™׳›׳”')).sort((a,b) => b.price - a.price);
  let finalLevels = [];
...
function detectAllPatterns(chartPoints) {
  if (!chartPoints || chartPoints.length < 50) return { text: "׳׳™׳ ׳׳¡׳₪׳™׳§ ׳ ׳×׳•׳ ׳™׳", lines: [] };
  ...
  if (curr.ma50 && curr.ma200 && prev.ma50 && prev.ma200) {
    if (prev.ma50 <= prev.ma200 && curr.ma50 > curr.ma200) patterns.push("׳—׳™׳×׳•׳ ׳–׳”׳‘ (Golden Cross) נ“ˆ");
    if (prev.ma50 >= prev.ma200 && curr.ma50 < curr.ma200) patterns.push("׳—׳™׳×׳•׳ ׳׳•׳•׳× (Death Cross) נ“‰");
  }
  ...
  if (Math.abs(h1 - h2) / h1 < 0.02 && currentPrice < h1 * 0.96) {
    patterns.push("׳₪׳¡׳’׳” ׳›׳₪׳•׳׳” (Double Top) ג›°ן¸ג›°ן¸");
  ...
  if (Math.abs(l1 - l2) / l1 < 0.02 && currentPrice > l1 * 1.04) {
    patterns.push("׳×׳—׳×׳™׳× ׳›׳₪׳•׳׳” (Double Bottom) נ•³ן¸נ•³ן¸");
  ...
  if (isRising && converge) {
    patterns.push("׳™׳×׳“ ׳¢׳•׳׳” (Rising Wedge) נ“ג¬‡ן¸");
    ...
    patterns.push("׳™׳×׳“ ׳™׳•׳¨׳“ (Falling Wedge) נ“ג¬†ן¸");
    ...
    patterns.push("׳×׳¢׳׳” ׳¢׳•׳׳” (Ascending Channel) ג†—ן¸");
    ...
    patterns.push("׳×׳¢׳׳” ׳™׳•׳¨׳“׳× (Descending Channel) ג†˜ן¸");
    lines.push({ start: {time: max1.time, value: h1}, end: {time: max2.time, value: h2}, color: 'rgba(10, 132, 255, 0.6)' });
    lines.push({ start: {time: min1.time, value: l1}, end: {time: min2.time, value: l2}, color: 'rgba(10, 132, 255, 0.6)' });
  }

  const text = patterns.length === 0 ? "׳“׳©׳“׳•׳© (Consolidation) ג†”ן¸" : [...new Set(patterns)].join(" | ");
  return { text, lines };
}
...
// =====================================================================
// CACHE: Two-layer caching - full (7d) and partial (24h for short-term)
// =====================================================================
const analysisCache = {};
const FULL_REFRESH_MS  = 7  * 24 * 60 * 60 * 1000;
const PARTIAL_REFRESH_MS = 24 * 60 * 60 * 1000;

function getCached(ticker, currPrice) {
  const entry = analysisCache[ticker];
  if (!entry) return null;
  const now = Date.now();
  const age = now - entry.timestamp;
  const priceChanged = Math.abs(currPrice - entry.price) / entry.price > 0.05;
  if (age > FULL_REFRESH_MS || priceChanged) return null;
  if (age <= PARTIAL_REFRESH_MS) return { type: 'full', data: entry.data };
  return { type: 'partial', data: entry.data };
}

function setCache(ticker, price, data) {
  analysisCache[ticker] = { timestamp: Date.now(), price, data };
}
...
[
{"ticker": "NVDA", "weight": 30, "role": "׳׳ ׳•׳¢ ׳¦׳׳™׳—׳” ׳•׳׳•׳׳ ׳˜׳•׳", "reason": "׳׳•׳‘׳™׳׳× ׳©׳•׳§ ׳”׳©׳‘׳‘׳™׳ ׳”׳¢׳•׳׳׳™׳×"},
{"ticker": "AAPL", "weight": 20, "role": "׳¢׳•׳’׳ ׳¢׳¨׳ ׳•׳™׳¦׳™׳‘׳•׳×", "reason": "׳×׳–׳¨׳™׳ ׳׳–׳•׳׳ ׳™׳ ׳׳“׳™׳¨"}
]`;
      ...
      try {
        ...
        return res.status(200).json({ success: true, portfolio: [
          {"ticker": "NVDA", "weight": 30, "role": "׳׳ ׳•׳¢ ׳¦׳׳™׳—׳” ׳•׳׳•׳׳ ׳˜׳•׳", "reason": "׳©׳׳™׳˜׳” ׳‘׳×׳—׳•׳ ׳”׳‘׳™׳ ׳” ׳”׳׳׳׳›׳•׳×׳™׳×"},
          {"ticker": "PLTR", "weight": 20, "role": "׳—׳“׳©׳ ׳•׳× ׳“׳׳˜׳”", "reason": "׳—׳•׳–׳™׳ ׳׳׳©׳׳×׳™׳™׳ ׳—׳–׳§׳™׳"},
          {"ticker": "MSFT", "weight": 20, "role": "׳¢׳•׳’׳ ׳•׳™׳¦׳™׳‘׳•׳×", "reason": "׳×׳–׳¨׳™׳ ׳™׳¦׳™׳‘ ׳•׳׳•׳“׳ ׳׳ ׳•׳™׳™׳"},
          {"ticker": "CRWD", "weight": 15, "role": "׳¡׳™׳™׳‘׳¨", "reason": "׳‘׳™׳§׳•׳© ׳§׳©׳™׳— ׳׳”׳’׳ ׳× ׳¢׳ ׳"},
          {"ticker": "TSLA", "weight": 15, "role": "׳×׳ ׳•׳“׳×׳™׳•׳×", "reason": "׳₪׳•׳˜׳ ׳¦׳™׳׳ ׳׳׳”׳׳›׳™׳ ׳׳”׳™׳¨׳™׳"}
        ]});
      }
    }
      ...
      return res.status(200).json({
        success: true, marketData: {
          ...
          sectors: [
            ...
            { sector: "׳›׳׳׳™", changesPercentage: String(spy?.dp || "0") }
          ],
          gainers: topGainers, losers: topLosers,
          news: (Array.isArray(newsData) ? newsData : []).slice(0, 5).map(item => ({
            title: item.headline || "׳׳׳ ׳›׳•׳×׳¨׳×", url: item.url || "#", source: item.source || "׳©׳•׳§ ׳”׳”׳•׳", date: new Date(item.datetime * 1000).toISOString()
          }))
        }
      });
    }
    ...
    if (!quote || quote.c === 0 || chartPoints.length < 10) {
      return res.status(404).json({ success: false, message: `׳”׳¡׳™׳׳•׳ ${ticker} ׳׳ ׳ ׳׳¦׳, ׳׳• ׳©׳׳™׳ ׳׳¡׳₪׳™׳§ ׳ ׳×׳•׳ ׳™ ׳׳¡׳—׳¨ ׳¢׳‘׳•׳¨׳•.` });
    }
    ...
    const insiderSentiment = netInsiderShares > 0 ? '׳—׳™׳•׳‘׳™ (׳§׳ ׳™׳•׳×)' : netInsiderShares < 0 ? '׳©׳׳™׳׳™ (׳׳›׳™׳¨׳•׳×)' : '׳ ׳™׳˜׳¨׳׳™';
    ...
    const rsiWeekAgo = closes.length > 5 ? calculateRSI(closes.slice(0, -5)) : rsiVal;
    const rsiTrendStr = rsiVal > rsiWeekAgo ? 'ג†‘ ׳¢׳•׳׳”' : rsiVal < rsiWeekAgo ? 'ג†“ ׳™׳•׳¨׳“' : 'ג†’ ׳™׳¦׳™׳‘';

    // ׳׳’׳׳× MA50
    const ma50Recent = chartPoints.slice(-5).map(p => p.ma50).filter(Boolean);
    const ma50Slope = ma50Recent.length > 1
      ? ((ma50Recent[ma50Recent.length - 1] - ma50Recent[0]) / ma50Recent[0] * 100).toFixed(2)
      : '0';

    // ׳•׳•׳׳™׳•׳ ׳™׳—׳¡׳™
    const avgVol30 = chartPoints.slice(-30).reduce((s, p) => s + (p.volume || 0), 0) / 30;
    const todayVol = lastPoint.volume || 0;
    const volRatio = avgVol30 > 0 ? (todayVol / avgVol30).toFixed(2) : 'N/A';

    const isDataComplete = true;
    const currPrice = Number(quote.c);

    // ׳‘׳“׳™׳§׳× cache ׳“׳•-׳©׳›׳‘׳×׳™
    const cached = getCached(ticker, currPrice);
    if (cached && cached.type === 'full') {
      return res.status(200).json({
        success: true, isDataComplete, ticker,
        name: profile?.name || ticker, industry: profile?.finnhubIndustry || "N/A", sector: profile?.finnhubIndustry || "N/A",
        ...
        analysis: cached.data, chartData: chartPoints
      });
    }
    ...
    // ׳¢׳•׳¦׳׳” ׳™׳—׳¡׳™׳×
    const stockChange = Number(quote.dp) || 0;
    const sectorChange = sectorQuote?.dp || 0;
    const spyChange = spyQuote?.dp || 0;
    const relativeStrength = (stockChange - sectorChange).toFixed(2);
    const marketContext = `׳”-S&P 500 (SPY) ׳”׳©׳×׳ ׳” ׳”׳™׳•׳ ׳‘-${spyChange.toFixed(2)}%. ׳¡׳§׳˜׳•׳¨ ׳”-${profile?.finnhubIndustry || '׳›׳׳׳™'} (${sectorETF}) ׳”׳©׳×׳ ׳” ׳‘-${sectorChange.toFixed(2)}%. ׳¢׳•׳¦׳׳” ׳™׳—׳¡׳™׳× ׳©׳ ${ticker} ׳׳•׳ ׳”׳¡׳§׳˜׳•׳¨: ${relativeStrength > 0 ? '+' : ''}${relativeStrength}%.`;

    let earningsStreak = "׳׳™׳ ׳׳¡׳₪׳™׳§ ׳ ׳×׳•׳ ׳™׳ ׳¢׳ ׳“׳•׳—׳•׳× ׳¢׳‘׳¨.";
    if (Array.isArray(earningsData) && earningsData.length >= 3) {
      const recent = earningsData.slice(0,3).map(e => e.surprisePercent > 0 ? 'ג…' : 'ג').join(' ');
      earningsStreak = `׳¨׳¦׳£ ׳”׳₪׳×׳¢׳•׳× ׳‘-3 ׳¨׳‘׳¢׳•׳ ׳™׳ ׳׳—׳¨׳•׳ ׳™׳: ${recent} (׳”׳“׳•׳— ׳”׳׳—׳¨׳•׳: ${earningsData[0].surprisePercent || 0}%).`;
    }

    // ׳—׳“׳©׳•׳× ׳׳§׳•׳¦׳¨׳•׳× - ׳—׳™׳¡׳›׳•׳ ׳‘׳˜׳•׳§׳ ׳™׳
    const recentNews = (Array.isArray(tickerNews) ? tickerNews : [])
      .slice(0, 3)
      .map(n => n.headline.slice(0, 60))
      .join(" | ");

    // partial cache logic
    let promptMode = 'full';
    let cachedLongTerm = null;
    let cachedIdentity = null;
    if (cached && cached.type === 'partial') {
      promptMode = 'partial';
      cachedLongTerm = cached.data.long_term;
      cachedIdentity = cached.data.identity;
    }

    const partialInstruction = promptMode === 'partial'
      ? `׳׳›׳™׳•׳•׳ ׳©׳”׳ ׳™׳×׳•׳— ׳”׳׳¨׳•׳-׳˜׳•׳•׳— ׳ ׳©׳׳¨ ׳‘-cache, ׳”׳—׳–׳¨ JSON ׳¢׳ ׳”׳©׳“׳•׳× ׳”׳‘׳׳™׳ ׳‘׳׳‘׳“ (partial refresh):
"short_term", "scores", "rating", "pros", "cons", "technical", "internal_logic".
׳׳ ׳×׳—׳–׳™׳¨ "identity" ׳•-"long_term" - ׳”׳ ׳™׳™׳˜׳¢׳ ׳• ׳׳”-cache.`
      : `׳”׳—׳–׳¨ JSON ׳׳׳ ׳¢׳ ׳›׳ ׳”׳©׳“׳•׳× ׳”׳׳₪׳•׳¨׳˜׳™׳.`;

    const prompt = `׳׳×׳” "FinShark" - ׳׳ ׳׳™׳¡׳˜ ׳׳ ׳™׳•׳× ׳•׳¡׳•׳—׳¨ ׳¡׳•׳•׳™׳ ׳’ ׳‘׳›׳™׳¨ ׳‘׳•׳•׳ ׳¡׳˜׳¨׳™׳˜.
׳”׳׳ ׳™׳”: ${ticker} ($${currPrice}).
׳׳¦׳‘ ׳”׳©׳•׳§ ׳”׳™׳•׳: ${marketContext}
׳—׳“׳©׳•׳× ׳׳—׳¨׳•׳ ׳•׳×: ${recentNews || '׳׳™׳ ׳—׳“׳©׳•׳× ׳׳™׳•׳—׳“׳•׳×'}

׳ ׳×׳•׳ ׳™׳ ׳˜׳›׳ ׳™׳™׳ (׳—׳•׳‘׳” ׳׳‘׳¡׳¡ ׳¢׳׳™׳”׳ ׳׳× ׳ ׳™׳×׳•׳— ׳”׳’׳¨׳£):
- ׳׳׳•׳¦׳¢׳™׳: ׳׳ ׳™׳” ׳‘-$${currPrice}, MA50: $${lastPoint.ma50} (slope 5d: ${ma50Slope}%), MA200: $${lastPoint.ma200}.
- ׳׳•׳׳ ׳˜׳•׳: RSI ${rsiVal.toFixed(1)} (${rsiTrendStr} ׳׳¢׳•׳׳× ׳׳₪׳ ׳™ 5 ׳™׳׳™׳: ${rsiWeekAgo.toFixed(1)}).
- ׳ ׳₪׳— ׳׳¡׳—׳¨: ${volRatio}x ׳׳׳•׳¦׳¢ 30 ׳”׳™׳׳™׳ ׳”׳׳—׳¨׳•׳ ׳™׳.
- ׳×׳‘׳ ׳™׳× ׳‘׳’׳¨׳£: ${patternObj.text}.
- ׳×׳׳™׳›׳”/׳”׳×׳ ׳’׳“׳•׳× ׳§׳¨׳•׳‘׳•׳×: ${levelsPrompt || '׳׳ ׳–׳•׳”׳•'}.

׳₪׳•׳ ׳“׳׳ ׳˜׳׳¡: P/E: ${fundamentals.peRatio || 'N/A'}, ׳¦׳׳™׳—׳× ׳”׳›׳ ׳¡׳•׳×: ${fundamentals.revenueGrowth || 'N/A'}%, ROE: ${fundamentals.roe || 'N/A'}%, ׳—׳•׳‘/׳”׳•׳: ${fundamentals.debtToEquity || 'N/A'}. ${earningsStreak}

TIMEFRAME CONTEXT: Swing = 2-8 ׳©׳‘׳•׳¢׳•׳×. Value = 6-18 ׳—׳•׳“׳©׳™׳. ׳׳ ׳×׳‘׳׳‘׳ ׳‘׳™׳ ׳™׳”׳ ׳׳¢׳•׳׳.
...
${promptMode === 'full' ? `׳”׳—׳–׳¨ JSON ׳‘׳׳‘׳“, ׳‘׳¢׳‘׳¨׳™׳×, ׳׳׳ ׳”׳¢׳¨׳•׳×:
{
  "internal_logic": "׳׳©׳₪׳˜ ׳‘׳׳ ׳’׳׳™׳× ׳¢׳ ׳”׳—׳׳˜׳× ׳”׳“׳™׳¨׳•׳’",
  "identity": "׳×׳™׳׳•׳¨ ׳¢׳¡׳§׳™ ׳§׳¦׳¨ ׳©׳ ׳”׳—׳‘׳¨׳”",
  "technical": "׳ ׳™׳×׳•׳— ׳˜׳›׳ ׳™ ׳˜׳”׳•׳¨ (׳׳׳ ׳₪׳•׳ ׳“׳׳ ׳˜׳׳¡!)",
  "long_term": { "summary": "׳׳¡׳§׳ ׳× ׳¢׳¨׳ (׳›׳׳ ׳©׳׳‘ ׳—׳“׳©׳•׳× ׳•׳₪׳•׳ ׳“׳׳ ׳˜׳׳¡)", "intrinsic_value": "׳©׳•׳•׳™ ׳”׳•׳’׳", "accumulation_zone": "׳˜׳•׳•׳— ׳׳™׳¡׳•׳£", "price_target": "׳™׳¢׳“ 12M" },
  "short_term": { "summary": "׳×׳•׳›׳ ׳™׳× ׳¡׳•׳•׳™׳ ׳’", "entry_price": "${currPrice}", "target_price": "׳™׳¢׳“ ׳˜׳›׳ ׳™", "stop_loss": "׳¡׳˜׳•׳₪ ׳׳•׳¡" },
  "pros": ["׳–׳›׳•׳× 1", "׳–׳›׳•׳× 2"], "cons": ["׳×׳•׳¨׳₪׳” 1", "׳×׳•׳¨׳₪׳” 2"],
  "scores": {"overall": 80, "growth": 80, "value": 80, "momentum": 80, "quality": 80},
  "rating": "׳§׳ ׳™׳™׳” / ׳”׳—׳–׳§׳” / ׳׳›׳™׳¨׳”"
}` : `׳”׳—׳–׳¨ JSON ׳—׳׳§׳™ ׳‘׳׳‘׳“ (partial refresh), ׳‘׳¢׳‘׳¨׳™׳×:
{
  "internal_logic": "...",
  "technical": "...",
  "short_term": { "summary": "...", "entry_price": "${currPrice}", "target_price": "...", "stop_loss": "..." },
  ...
  "rating": "׳§׳ ׳™׳™׳” / ׳”׳—׳–׳§׳” / ׳׳›׳™׳¨׳”"
    ...
    let aiVerdict = {
      ...
      identity: "׳©׳’׳™׳׳” ׳‘׳©׳¨׳× ׳”׳›׳¨׳™׳©, ׳׳ ׳ ׳™׳×׳ ׳׳ ׳×׳— ׳׳× ׳”׳׳ ׳™׳” ׳›׳¨׳’׳¢.",
      technical: "׳™׳›׳•׳ ׳׳”׳™׳•׳× ׳©׳”׳׳›׳¡׳” ׳©׳ ׳”-API ׳”׳¡׳×׳™׳™׳׳” ׳׳• ׳©׳™׳© ׳×׳§׳׳× ׳¨׳©׳×. ׳׳ ׳ ׳ ׳¡׳” ׳©׳•׳‘ ׳׳׳•׳—׳¨ ׳™׳•׳×׳¨.",
      long_term: { summary: "׳׳ ׳–׳׳™׳", intrinsic_value: "N/A", accumulation_zone: "N/A", price_target: "N/A" },
      short_term: { summary: "׳׳ ׳–׳׳™׳", entry_price: "N/A", target_price: "N/A", stop_loss: "N/A" },
      scores: null, rating: "׳©׳’׳™׳׳”", pros: ["׳©׳’׳™׳׳”"], cons: ["׳©׳’׳™׳׳”"]
    };
    ...
    try {
      ...
      if (aiResponse.ok) {
        ...
        if (aiData.candidates && aiData.candidates[0].content) {
          ...
          if (promptMode === 'partial' && cachedLongTerm && cachedIdentity) {
            aiVerdict = { ...parsed, identity: cachedIdentity, long_term: cachedLongTerm, isError: false };
          } else {
            aiVerdict = { ...parsed, isError: false };
          }

          setCache(ticker, currPrice, aiVerdict);
        }
      } else {
